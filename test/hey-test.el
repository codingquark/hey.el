...
(ert-deftest hey-ui-thread-link-preview-preserves-full-destination ()
  "Clip previews without changing copied, opened, or tracked destinations."
  (dolist (suffix '("short" "界界界界界界界界界界界界界界界界界界界界%2F?q=full"))
    (let ((destination (concat "https://example.invalid/" suffix)))
      (hey-test-with-thread-body (format "Read [target](%s)." destination)
        (let ((hey-link-echo-max-width 50)
              copied opened echoed logged)
          (cl-letf (((symbol-function 'hey--minimum-window-width)
                     (lambda () 45))
                    ((symbol-function 'kill-new)
                     (lambda (url &rest _) (setq copied url)))
                    ...
                    ((symbol-function 'message)
                     (lambda (format-string &rest args)
                       (setq logged message-log-max
                             echoed (apply #'format format-string args)))))
            (hey-test--point-at-link "target")
            (hey-show-link)
            (should-not logged)
            (should (<= (string-width echoed) 45))
            (if (equal suffix "short")
                (should (equal echoed (concat "Link: " destination)))
              (should (string-suffix-p "…" echoed))
              (should (string-prefix-p (substring echoed 6 -1) destination)))
            (should (equal hey--echoed-url destination))
            (hey-copy-link)
            (should (equal copied destination))
            (hey-follow-link)
            (should (equal opened destination))
            (should (equal hey--echoed-url destination))
            ;; The configured cap also applies in wider windows.
            (setq hey-link-echo-max-width 35)
            (hey-show-link)
            (should (<= (string-width echoed) 35))
            (dolist (width '(1 5 6 7 0 -1))
              (setq hey-link-echo-max-width width)
              (hey-show-link)
              (should (<= (string-width echoed) (max 1 width))))))))))
